...
typedef struct upb_TaggedAuxPtr {
  ...
  // Bit 2 (highest bits): Aliased/Non-aliased (1 for aliased, 0 for
  //   non-aliased).
  //
...
// Returns true if the entry is aliased or unaliased unknown upb_StringView*
// data.
UPB_INLINE bool upb_TaggedPtrAux_HasStringViewRepr(upb_TaggedAuxPtr ptr) {
...
UPB_INLINE upb_StringView* upb_TaggedPtrAux_StringViewRepr(
    upb_TaggedAuxPtr ptr) {
  UPB_ASSERT(upb_TaggedPtrAux_HasStringViewRepr(ptr));
...
UPB_NODISCARD UPB_INLINE bool UPB_PRIVATE(
  ...
  if (in && in->size) {
    ...
    if (upb_TaggedPtrAux_HasStringViewRepr(ptr)) {
...
UPB_INLINE bool upb_Message_NextUnknown(const struct upb_Message* msg,
  ...
  if (in) {
    while (i < in->size) {
      ...
      if (upb_TaggedPtrAux_HasStringViewRepr(tagged_ptr)) {
