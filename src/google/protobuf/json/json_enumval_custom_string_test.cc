...
TEST(JsonEnumvalCustomStringTest, ParserSingleElementArrayCustomName) {
  Knight msg;
  absl::Status status =
      json::JsonStringToMessage(R"json({"armor":["gr8 helm"]})json", &msg);
  EXPECT_THAT(status, StatusIs(absl::StatusCode::kInvalidArgument,
                               HasSubstr("invalid JSON")));
}

TEST(JsonEnumvalCustomStringTest, ParserSingleElementArrayEnumNumberString) {
  Knight msg;
  absl::Status status =
      json::JsonStringToMessage(R"json({"armor":["1"]})json", &msg);
  EXPECT_THAT(status, StatusIs(absl::StatusCode::kInvalidArgument,
                               HasSubstr("invalid JSON")));
}
