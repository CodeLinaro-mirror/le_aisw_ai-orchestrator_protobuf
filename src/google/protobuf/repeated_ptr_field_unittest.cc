...
TEST_F(RepeatedPtrFieldTest, NaturalGrowthOnArenasGrowsTheTail) {
  ...
  for (int i = 0; i < kNumFields; ++i) {
    ...
    // Sometimes we pushed the initial block into the free lists, which will
    // pollute the rest of the results. When this happens, the following
    // containers will always start with the free-list block, the grow into the
    // tail, skewing the results. Let's eat that block if it is there.
    // Eat the blocks for 32-bit and 64-bit.
