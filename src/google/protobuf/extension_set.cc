...
const ExtensionSet::FlatItem ExtensionSet::kEmptyKeyValue = {};

[[maybe_unused]] const bool ExtensionSet::kPoisonEmptyKeyValue = [] {
  PoisonMemoryRegion(&ExtensionSet::kEmptyKeyValue.first,
                     sizeof(ExtensionSet::kEmptyKeyValue.first));
  PoisonMemoryRegion(&ExtensionSet::kEmptyKeyValue.second,
                     sizeof(ExtensionSet::kEmptyKeyValue.second));
  return true;
}();
...
ExtensionSet::FlatItem* ExtensionSet::AllocateFlatMap(
  ...
  auto* flat =
      Arena::CreateArray<ExtensionSet::FlatItem>(arena, powerof2_flat_capacity);
  return flat;
}
...
ABSL_ATTRIBUTE_NOINLINE void ExtensionSet::InternalReduceSmallCapacity(
  ...
  map_.flat = new_flat;
  set_flat_capacity(new_flat_capacity);
  set_flat_size(current_size);
}
...
void ExtensionSet::InternalMergeFromSmallToEmpty(Arena* arena,
  ...
  ABSL_ASSUME(static_cast<int16_t>(new_size) > 0);
...
std::pair<ExtensionSet::Extension*, bool> ExtensionSet::Insert(Arena* arena,
  ...
  set_flat_capacity(current_cap);
  set_flat_size(current_size + 1);
...
void ExtensionSet::GrowCapacity(Arena* arena, size_t minimum_new_capacity) {
  ...
  if (new_flat_capacity > kMaximumFlatCapacity) {
    ...
    new_map.flat[0].flat_capacity = static_cast<uint16_t>(new_flat_capacity);
    new_map.flat[0].flat_size = current_size;
  }
