...
// and partialMergeFrom implementations are recursively split into subfunctions
// to avoid exceeding the Java 64KB method byte-code size limit.
...
void MessageBuilderGenerator::GenerateBuilderMergeFromSubfunction(
  ...
  int block_count = merging_code_blocks.size();
  if (block_count <= kMergeMethodSplitThreshold) {
    ...
    // Recursively partition the snippets into smaller chunks if they still
    // exceed the threshold.
    int chunk_count = std::min(kMergeMethodSplitThreshold,
                               (block_count + kMergeMethodSplitThreshold - 1) /
                                   kMergeMethodSplitThreshold);
    int base_chunk_size = block_count / chunk_count;
    int remainder_elements = block_count % chunk_count;
    int current_block_index = 0;
    ...
    for (int i = 0; i < chunk_count; ++i) {
      int current_chunk_size =
          base_chunk_size + (i < remainder_elements ? 1 : 0);
      std::string child_method_suffix = absl::StrCat(method_suffix, "_", i);
      subfunction_chunks.push_back(
          {current_block_index, current_chunk_size, child_method_suffix});
      current_block_index += current_chunk_size;
    }

    // Call the next level of generated subfunctions.
    for (const auto& chunk : subfunction_chunks) {
      printer->Print("partialMergeFrom$suffix$(other);\n", "suffix",
                     chunk.method_suffix);
    }
    ...
    // Recursively generate the subfunctions for the chunks.
    for (const auto& chunk : subfunction_chunks) {
      GenerateBuilderMergeFromSubfunction(
          printer, merging_code_blocks.subspan(chunk.start_index, chunk.size),
          chunk.method_suffix);
    }
  }
}
