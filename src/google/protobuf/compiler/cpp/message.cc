...
void MessageGenerator::EmitMergeChunks(io::Printer* p, bool is_split) {
...
void MessageGenerator::GenerateClassSpecificMergeImpl(io::Printer* p) {
  ...
  p->Emit(
      {{"has_bit_consistency",
       ...
       {"merge_fields", [&] { EmitMergeChunks(p, false); }},
       {"merge_split",
        [&] {
          if (!ShouldSplit(descriptor_, options_)) return;
          p->Emit(R"cc(
          ...
          [&] {
            ...
            p->Emit(
                {{"destroy_fields",
                 ...
                 {"merge_fields", [&] { EmitMergeChunks(p, true); }},
                 {"clear_fields", [&] { EmitClearChunks(p, true); }},
                 {"byte_size_fields", [&] { EmitByteSizeChunks(p, true); }}},
                R"cc(
