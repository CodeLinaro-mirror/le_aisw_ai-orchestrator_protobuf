  ...
  ($(($type: ident, $name_ext: ident)),*) => {
        paste! {$(
            ...
            fn [<expect_partially_eq_msgs_ $name_ext>]() {
                ...
                assert_that!(&actual.as_view(), partially(proto_eq(expected.as_view())));
                assert_that!(&actual, partially(proto_eq(&expected)));

                // missing field in actual -> does NOT match partially
                let mut missing_actual = [< $type >]::new();
                missing_actual.set_optional_int64(42);
                assert_that!(&missing_actual, not(partially(proto_eq(&expected))));
                ...
                assert_that!(&diff_actual, not(partially(proto_eq(&expected))));
            }
            ...
            fn [<expect_partially_presence_field_set_to_default_ $name_ext>]() {
                ...
                assert_that!(&actual_empty, not(partially(proto_eq(&expected))));
                ...
                assert_that!(&actual_matching, partially(proto_eq(&expected)));
            }
            ...
            fn [<expect_partially_nested_message_ $name_ext>]() {
                ...
                assert_that!(&diff_nested, not(partially(proto_eq(&expected))));
            }
            ...
            fn [<expect_partially_repeated_fields_ $name_ext>]() {
                ...
                assert_that!(&diff_repeated, not(partially(proto_eq(&expected))));
            }
        )*}
    }
}
...
#[gtest]
fn test_partially_extension_matching() {
    let mut actual = extensions_rust_proto::TestExtensions::new();
    let mut expected = extensions_rust_proto::TestExtensions::new();

    extensions_rust_proto::I32_EXTENSION.set(&mut actual, 42);
    extensions_rust_proto::STR_EXTENSION.set(&mut actual, "hello");

    extensions_rust_proto::I32_EXTENSION.set(&mut expected, 42);

    // actual has an extra extension (STR_EXTENSION) -> matches partially
    assert_that!(&actual.as_view(), partially(proto_eq(expected.as_view())));
    assert_that!(&actual, partially(proto_eq(&expected)));

    // diff value for I32_EXTENSION -> does NOT match partially
    let mut diff_actual = extensions_rust_proto::TestExtensions::new();
    extensions_rust_proto::I32_EXTENSION.set(&mut diff_actual, 99);
    extensions_rust_proto::STR_EXTENSION.set(&mut diff_actual, "hello");
    assert_that!(&diff_actual, not(partially(proto_eq(&expected))));

    // missing I32_EXTENSION in actual -> does NOT match partially
    let mut missing_actual = extensions_rust_proto::TestExtensions::new();
    extensions_rust_proto::STR_EXTENSION.set(&mut missing_actual, "hello");
    assert_that!(&missing_actual, not(partially(proto_eq(&expected))));
}
